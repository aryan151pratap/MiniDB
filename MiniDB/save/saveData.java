...
public class saveData {
    ...
    public static String loadDatabaseNames(Map<String, Map<String, ArrayList<Map<String, Object>>>> db) {
        ...
        if (!folder.exists()) return "No databases found!";
    ...
    public static String saveDatabase(String dbName,
        ...
        try {
            if (!db.containsKey(dbName)) {
                return "Database '" + dbName + "' does not exist in memory!";
            }
            File folder = new File("data");
            if (!folder.exists()) folder.mkdir();
            String fileName = "data/" + dbName + ".db";
            File file = new File(fileName);
            if (file.exists()) {
                return "Database '" + dbName + "' already exists on disk!";
            }
            try (ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream(file))) {
                oos.writeObject(db.get(dbName));
            }
            return "Database '" + dbName + "' saved to " + fileName;
        } catch (IOException e) {
    ...
    public static String saveCollection(String current_db, String collection,
        ...
        try {
            if (!db.containsKey(current_db)) {
                return "Database '" + current_db + "' does not exist in memory!";
            }
            Map<String, ArrayList<Map<String, Object>>> collections = db.get(current_db);

            if (!collections.containsKey(collection)) {
                return "Collection '" + collection + "' does not exist in database '" + current_db + "'!";
            }

            File folder = new File("data");
            if (!folder.exists()) folder.mkdir();

            String fileName = "data/" + current_db + ".db";
            
            try (ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream(fileName))) {
                oos.writeObject(db.get(current_db));
            }

            return "Collection '" + collection + "' saved in database '" + current_db + "' (" + fileName + ")";
        } catch (IOException e) {
    ...
    public static String Drop_db(String db_name){
        try{
            String fileName = "data/" + db_name + ".db";
            File file = new File(fileName);

            if (file.exists()) {
                if (file.delete()) {
                    return "Database '" + db_name + "' dropped and file deleted (" + fileName + ")";
                } else {
                    return "Database '" + db_name + "' removed from memory but failed to delete file!";
                }
            } else {
    ...
    public static String loadDatabase(String dbName,
        ...
        try {
            File folder = new File("data");
            if (!folder.exists()) return "Data folder not found!";

            String fileName = "data/" + dbName + ".db";
            File file = new File(fileName);

            if (!file.exists()) {
                return "File for database '" + dbName + "' not found!";
            }
            ...
            return "Database '" + dbName + "' loaded from " + fileName;
